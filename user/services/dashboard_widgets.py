...
def _broiler_activity(viewable, filters, user=None):
    ...
    The gathering itself lives in broiler.services.activity_log, shared with
    the full Recent Activity Log page this card links to — one definition of
    what counts as an event and how it is scoped, so the card and the page
    ...
    # same row rather than sharing one.
    timeline = []
    for event in events[:12]:
        row = {k: v for k, v in event.items() if k != "when"}
        row["time_ago"] = activity_ago(as_datetime(event["when"]))
        timeline.append(row)

    present = {e["category"] for e in events}
    timeline_filters = [{"key": "all", "label": "All Activities"}] + [
        {"key": k, "label": label} for k, label, _colour in CATEGORIES if k in present]

    footer_stats = [
        {"label": "Total Activities", "value": _num(len(events)),
         "sub": "in the last 7 days"},
        {"label": "Last Activity", "value": timeline[0]["time_ago"],
         "sub": f"{timeline[0]['title']} — {timeline[0]['farm']}"},
        {"label": "Stock Alerts", "value": _num(stock_alerts),
         "tone": "bad" if stock_alerts else "good"},
    ]
    if total_farms:
        footer_stats.append({
